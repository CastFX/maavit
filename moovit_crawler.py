...
class MoovitUrl:
    ...
    BASE_URL_MISS = 'https://moovitapp.com'
...
class MoovitCrawler:
    ...
    def _initiate_session(self):
        headers = {
            ...
            'From': MoovitUrl.BASE_URL_MISS
        }

        if not self.session:
            self.session = requests.Session()
            resp = self.session.get(MoovitUrl.BASE_URL, headers=headers)
            return resp
    ...
    def get_rzbid(self) -> str:
        ...
        try:
            ...
            print("Check Your Connection, No resp_id" + err)
        print(MoovitUrl.BASE_URL_MISS + id[0])
        content_id = BeautifulSoup(resp_id.content, 'html.parser')
        content_text = content_id.text
        print(content_id)
        matches = re.findall(r"window\.rbzid=\'(.+?)\'", content_text)
        try:
            print(matches[0])
            return matches[0]
        except IndexError as err_in:
            print("Check Your Connection, No resp_id" + err_in)
